...
### @brief ADC2019 の問題をCNF論理式にエンコードするクラス
class Adc2019Enc :
    ...
    def get_answer(self, model) :
        ...
        # 線分を取り出す．
        for line_id in self.__problem.line_id_list :
            (block_id1, pos1), (block_id2, pos2) = self.__problem.terminals(line_id)
    ...
    ### @brief 条件付きの One-Hot 制約を作る．
    ### @param[in] var_list 対象の変数のリスト
    ### @param[in] cond 条件となるリテラル
    ...
    ### @brief グリッドのブロック番号を表す変数を返す．
    ### @param[in] pos グリッドの位置
    ### @param[in] block_id ブロック番号
    ### @return 対象の変数(SatLiteral)を返す．
    ###
